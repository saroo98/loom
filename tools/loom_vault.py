#!/usr/bin/env python3
"""Loom 1.1 encrypted, transactional, owner-scoped state authority.
...
CREATE INDEX memory_scope_index ON memory_records(scope, domain_tag, project_tag, status);
...
class OwnerVault:
    ...
    def _validate_record(self, record):
        ...
        if record["scope"] == "global" and (record["domain"] is not None
                                               or record["project_id"] is not None):
            raise VaultError("global memory cannot carry domain or project identity")
    ...
    def put_memory(self, record, *, source_sequence=0):
        ...
        domain_tag, project_tag = self._tags(record)
        semantic_tag = self._semantic_tag(record)
        aad = f"memory:{self.identity()['owner_vault_id']}:{record['id']}".encode()
        ...
        def write(connection):
            ...
            if record["category"] == "preference":
                candidates = connection.execute(
                    ...
                    "AND domain_tag IS ? AND project_tag IS ? AND record_id!=?",
                    (record["scope"], domain_tag, project_tag, record["id"])).fetchall()
                for candidate in candidates:
                    prior = self._decrypt_record(candidate)
                    if prior.get("category") == "preference" \
                            and prior.get("preference_key") == record["preference_key"]:
                        connection.execute(
                            "UPDATE memory_records SET status='superseded',updated_at=? "
                            "WHERE record_id=?", (_stamp(), candidate["record_id"]))
            self._next_event(
                connection, kind="memory-upsert", payload=record, scope=record["scope"],
                domain_tag=domain_tag, project_tag=project_tag)
            connection.execute(
                "INSERT INTO memory_records(record_id,scope,domain_tag,project_tag,semantic_tag,"
                "status,source_sequence,ciphertext,updated_at) VALUES(?,?,?,?,?,?,?,?,?) "
                "ON CONFLICT(record_id) DO UPDATE SET scope=excluded.scope,"
                "domain_tag=excluded.domain_tag,project_tag=excluded.project_tag,"
                "semantic_tag=excluded.semantic_tag,status=excluded.status,"
                "source_sequence=excluded.source_sequence,ciphertext=excluded.ciphertext,"
                "updated_at=excluded.updated_at WHERE excluded.source_sequence >= "
                "memory_records.source_sequence",
                (record["id"], record["scope"], domain_tag, project_tag, semantic_tag,
                 record["status"], source_sequence, ciphertext, _stamp()))
            connection.execute(
    ...
    def import_tombstone(self, *, record_id, semantic_fingerprint, forgotten_at,
        ...
        payload = {"record_id": record_id, "legacy_fingerprint": semantic_fingerprint,
                   "forgotten_at": forgotten_at}
        aad = f"tombstone:{self.identity()['owner_vault_id']}:{record_id}".encode()
        ciphertext = self.crypto.seal(_canonical(payload), aad)
        ...
        def write(connection):
            ...
            self._next_event(
                connection, kind="legacy-tombstone-import", payload=payload,
                scope="vault", domain_tag=None, project_tag=None)
            connection.execute("DELETE FROM memory_records WHERE record_id=?", (record_id,))
            connection.execute(
                "INSERT INTO tombstones(record_id,semantic_tag,source_sequence,ciphertext,forgotten_at) "
                "VALUES(?,?,?,?,?) ON CONFLICT(record_id) DO UPDATE SET "
                "semantic_tag=excluded.semantic_tag,source_sequence=excluded.source_sequence,"
                "ciphertext=excluded.ciphertext,forgotten_at=excluded.forgotten_at",
                (record_id, tag, source_sequence, ciphertext, forgotten_at))
    ...
    def put_entity(self, entity_type, entity_id, payload, *, source_sequence=0):
        ...
        aad = f"entity:{owner}:{entity_type}:{entity_id}".encode()
        ciphertext = self.crypto.seal(raw, aad)

        def write(connection):
            exists = connection.execute(
                "SELECT 1 FROM state_entities WHERE entity_type=? AND entity_id=?",
            ...
            if exists is None:
                ...
                if type_count >= MAX_ENTITY_TYPE or total_count >= MAX_STATE_ENTITIES:
                    oldest = (connection.execute(
                        "SELECT entity_type,entity_id FROM state_entities WHERE entity_type=? "
                        "ORDER BY updated_at,entity_id LIMIT 1", (entity_type,)).fetchone()
                        if type_count >= MAX_ENTITY_TYPE else connection.execute(
                            "SELECT entity_type,entity_id FROM state_entities "
                            "ORDER BY updated_at,entity_type,entity_id LIMIT 1").fetchone())
                    if oldest is None:
                        raise VaultError("state entity bound reached")
                    connection.execute(
                        "DELETE FROM state_entities WHERE entity_type=? AND entity_id=?",
                        (oldest["entity_type"], oldest["entity_id"]))
            self._next_event(
                connection, kind="state-entity-upsert",
                payload={"entity_type": entity_type, "entity_id": entity_id,
                         "value": payload},
                scope="vault", domain_tag=None, project_tag=None)
            connection.execute(
                "INSERT INTO state_entities(entity_type,entity_id,source_sequence,ciphertext,updated_at) "
                "VALUES(?,?,?,?,?) ON CONFLICT(entity_type,entity_id) DO UPDATE SET "
                "source_sequence=excluded.source_sequence,ciphertext=excluded.ciphertext,"
                "updated_at=excluded.updated_at WHERE excluded.source_sequence >= "
                "state_entities.source_sequence",
                (entity_type, entity_id, source_sequence, ciphertext, _stamp()))
    ...
    def merge_events(self, events):
        ...
        def merge(connection):
            metadata = self._metadata(connection)
            receipt = {"added": 0, "deduplicated": 0, "forgotten": 0,
                ...
                if event["payload_schema_version"] != PAYLOAD_SCHEMA_VERSION:
                    ...
                    if kind == "memory-upsert":
                        ...
                        if (record["scope"], domain_tag, project_tag) != (
                            ...
                            raise VaultError("merge memory scope does not match signed envelope")
                        semantic_tag = self._semantic_tag(record)
                        if connection.execute(
                                "SELECT 1 FROM tombstones WHERE record_id=? OR semantic_tag=?",
                                (record["id"], semantic_tag)).fetchone():
                            ...
                            if record["category"] == "preference" and record["status"] == "active":
                                candidates = connection.execute(
                                    ...
                                    "AND COALESCE(project_tag,'')=COALESCE(?, '')",
                                    (record["scope"], domain_tag, project_tag)).fetchall()
                                owner = metadata["owner_vault_id"]
                                for candidate in candidates:
                                    candidate_aad = f"memory:{owner}:{candidate['record_id']}".encode()
                                    existing = self._validate_record(json.loads(
                                        self.crypto.open(candidate["ciphertext"], candidate_aad)
                                        .decode("utf-8")))
                                    if existing["category"] == "preference" \
                                            and existing["preference_key"] == record["preference_key"] \
                                            and existing["preference_value"] != record["preference_value"]:
                            ...
                            if conflicts:
                                ...
                                for candidate, existing in conflicts:
                                    ...
                                    existing_aad = f"memory:{metadata['owner_vault_id']}:" \
                                        f"{candidate['record_id']}".encode()
                                    connection.execute(
                                        "UPDATE memory_records SET status='dormant',ciphertext=?,updated_at=? "
                                        "WHERE record_id=?",
                                        (self.crypto.seal(_canonical(existing), existing_aad),
                            ...
                            connection.execute(
                                "INSERT INTO memory_records(record_id,scope,domain_tag,project_tag,"
                                "semantic_tag,status,source_sequence,ciphertext,updated_at) "
                                "VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(record_id) DO NOTHING",
                                (record["id"], record["scope"], domain_tag, project_tag,
                                 semantic_tag, record["status"], event["device_counter"],
                                 self.crypto.seal(_canonical(record), record_aad), _stamp()))
                            connection.execute(
                                "INSERT OR IGNORE INTO memory_utility(record_id) VALUES(?)",
                                (record["id"],))
                            receipt["added"] += 1
                    elif kind in {"memory-forgotten", "memory-expired",
                                  "memory-superseded-expired"}:
                        ...
                        tombstone_aad = f"tombstone:{metadata['owner_vault_id']}:{record_id}".encode()
                        tombstone_ciphertext = self.crypto.seal(_canonical(payload), tombstone_aad)
                        connection.execute(
                            "INSERT OR IGNORE INTO tombstones(record_id,semantic_tag,source_sequence,"
                            "ciphertext,forgotten_at) VALUES(?,?,?,?,?)",
                            (record_id, semantic_tag, event["device_counter"],
                             tombstone_ciphertext, payload["forgotten_at"]))
                        connection.execute(
                            "DELETE FROM memory_records WHERE record_id=? OR semantic_tag=?",
                            (record_id, semantic_tag))
                        receipt["forgotten"] += 1
                    elif kind == "memory-outcome":
                        if not isinstance(payload, dict) or set(payload) != {
                                "selected_ids", "helped_ids", "hurt_ids"}:
                            raise VaultError("memory outcome event payload is invalid")
                        selected = payload["selected_ids"]
                        helped = set(payload["helped_ids"])
                        hurt = set(payload["hurt_ids"])
                        if not isinstance(selected, list) or not all(
                                isinstance(item, str) for item in selected) \
                                or not helped <= set(selected) or not hurt <= set(selected) \
                                or helped & hurt:
                            raise VaultError("memory outcome event IDs are invalid")
                        for record_id in selected:
                            connection.execute(
                                "UPDATE memory_utility SET helped_count=helped_count+?,"
                                "hurt_count=hurt_count+?,last_helped=CASE WHEN ? THEN ? "
                                "ELSE last_helped END,last_hurt=CASE WHEN ? THEN ? "
                                "ELSE last_hurt END WHERE record_id=?",
                                (int(record_id in helped), int(record_id in hurt),
                                 int(record_id in helped), _stamp(),
                                 int(record_id in hurt), _stamp(), record_id))
                        receipt["recomputed"] += 1
                    elif kind == "state-entity-upsert":
                        if not isinstance(payload, dict) or set(payload) != {
                        ...
                        entity_aad = f"entity:{metadata['owner_vault_id']}:" \
                            f"{entity_type}:{entity_id}".encode()
                        connection.execute(
                            "INSERT OR IGNORE INTO state_entities(entity_type,entity_id,source_sequence,"
                            "ciphertext,updated_at) VALUES(?,?,?,?,?)",
                            (entity_type, entity_id,
                             event["device_counter"],
                             self.crypto.seal(_canonical(payload["value"]), entity_aad), _stamp()))
                        # Merge order must not change the bounded materialized view. Keep the
                        # same deterministic winners after every delivery order.
                        connection.execute(
                        ...
                        connection.execute(
                            ...
                            "SELECT entity_type,entity_id FROM state_entities "
                            "ORDER BY source_sequence DESC,entity_type,entity_id "
                            "LIMIT -1 OFFSET ?)", (MAX_STATE_ENTITIES,))
                        receipt["added"] += 1
                    else:
                        self._quarantine_connection(
    ...
    def select_memory(self, *, domain, project_id, max_records=16, max_chars=8000):
        if not isinstance(domain, str) or not domain or type(max_records) is not int \
                or type(max_chars) is not int or not 1 <= max_records <= 16 \
        ...
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT * FROM memory_records WHERE status='active' AND (scope='global' "
                "OR (scope='domain' AND domain_tag=?) "
                "OR (scope='project' AND domain_tag=? AND project_tag=?)) "
                "ORDER BY updated_at DESC,record_id",
                (domain_tag, domain_tag, project_tag)).fetchall()
        selected = []
        used = 0
        for row in rows:
            record = self._decrypt_record(row)
    ...
    def improvement_summary(self):
        with self._connect() as connection:
            utility = connection.execute(
                ...
                "COALESCE(SUM(hurt_count),0) FROM memory_utility").fetchone()
            outcomes = connection.execute(
                "SELECT COUNT(*) FROM state_entities WHERE entity_type='session-outcome'").fetchone()[0]
        selected, helped, hurt = map(int, utility)
    ...
    def compact_acknowledged(self):
        ...
        def write(connection):
            ...
            connection.execute(
                "DELETE FROM events WHERE recorded_at<=?", (checkpoint["created_at"],))
    ...
    def record_memory_outcome(self, selected_ids, *, helped_ids=(), hurt_ids=()):
        ...
        def write(connection):
            ...
            self._next_event(
                connection, kind="memory-outcome",
                payload={"selected_ids": selected, "helped_ids": sorted(helped),
    ...
    def maintain_memory_lifecycle(self, *, now=None):
        ...
        def write(connection):
            rows = connection.execute(
                ...
                "FROM memory_records m JOIN memory_utility u USING(record_id) "
                "WHERE m.scope IN ('domain','project') OR m.status='superseded'").fetchall()
            receipt = {"dormant": 0, "archived": 0, "expired": 0, "retained": 0}
            for row in rows:
                record = self._decrypt_record(row)
                reference = row["last_selected"] or record["created_at"]
                ...
                if status == "superseded" and age_days(row["updated_at"]) >= 90:
                    ...
                    aad = (f"tombstone:{self.identity()['owner_vault_id']}:"
                    ...
                    connection.execute(
                        "INSERT OR IGNORE INTO tombstones(record_id,semantic_tag,source_sequence,"
                        "ciphertext,forgotten_at) VALUES(?,?,?,?,?)",
                        (row["record_id"], row["semantic_tag"], row["source_sequence"],
                         ciphertext, payload["forgotten_at"]))
                    connection.execute(
                        "DELETE FROM memory_records WHERE record_id=?", (row["record_id"],))
                    receipt["expired"] += 1
                    continue
                if not explicit and status == "active" and age >= 30 \
                        and row["helped_count"] == 0:
                    next_status = "dormant"
                elif not explicit and status in {"dormant", "stale"} and age >= 90 \
                        and row["helped_count"] == 0:
                ...
                if next_status:
                    connection.execute(
                        ...
                        (next_status, _stamp(), row["record_id"]))
    ...
    def forget_memory(self, record_id, *, reason):
        ...
        def write(connection):
            ...
            connection.execute(
                "INSERT INTO tombstones(record_id,semantic_tag,source_sequence,ciphertext,forgotten_at) "
                "VALUES(?,?,?,?,?)",
                (record_id, row["semantic_tag"], row["source_sequence"], ciphertext,
        ...
        def rotate(connection):
            ...
            for row in connection.execute("SELECT * FROM memory_records").fetchall():
                ...
                body = {key: record.get(key) for key in (
                    "scope", "domain", "project_id", "category", "statement",
                    "preference_key", "preference_value")}
                if new_crypto.blind_index(
                        "memory-semantic", _canonical(body).decode("utf-8")) != row["semantic_tag"]:
            ...
            connection.execute(
                "INSERT INTO state_entities(entity_type,entity_id,source_sequence,ciphertext,updated_at) "
                "VALUES('device-key',?,?,?,?) ON CONFLICT(entity_type,entity_id) DO UPDATE SET "
                "ciphertext=excluded.ciphertext,updated_at=excluded.updated_at",
                (entity_id, 0, new_crypto.seal(_canonical(local_device_keys), aad), _stamp()))
